...
public class ZJoystoke : MonoBehaviour
{
    ...
    //--------------------------------------------AxisName-----------------------------------------------------
    [SerializeField] private string AxisName_H, AxisName_V , Direction;
    ...
    private void Awake()
    {
        ...
        if (Direction != "" && Direction3D.ContainsKey(Direction) == false)
        {
            Direction3D.Add(Direction, new Quaternion());
        }
        if (Direction != "" && Direction2D.ContainsKey(Direction) == false)
        {
            Direction2D.Add(Direction, new Quaternion());
        }
    }
    ...
    public void DragEvent(PointerEventData data)
    {
        ...
        if (Direction != "")
        {
            var Direction_3D = new Vector3(-Y, 0, X);
            Quaternion QuaternionDirection3D = Quaternion.LookRotation(Direction_3D);
            Direction3D[Direction] = QuaternionDirection3D;

            var Direction_2D = new Vector2(X, Y);
            Quaternion QuaternionDirection2D = Quaternion.LookRotation(Direction_2D);
            Direction2D[Direction] = QuaternionDirection2D;
        }
        else
        {
            Debug.LogError("DirAxisName Equals Null");
        }
    }
}
